...
library(datasets)
library(dplyr)
#apply mtcars dataset for demo purpose
shinyServer(function(input, output) {
        
        # Show the cars that correspond to the filters
        output$table <- renderDataTable({
                display_seq <- seq(from = input$disp[1], to = input$disp[2], by = 0.1)
                ...
                data <- filter(data, GasolineExpenditure <= input$gas, Cylinders %in% input$cyl, 
                               Displacement %in% display_seq, Horsepower %in% hp_seq, Transmission %in% input$am)
